...
# ✅ Temporary direct key (testing mate)
client = OpenAI(api_key="YOUR_REAL_API_KEY")

# 🔒 Privacy filter
def is_sensitive(msg):
    words = ["student data", "candidate list", "all users", "resume list"]
    return any(word in msg.lower() for word in words)
...
def smart_reply(msg):
    ...
    if "resume" in msg:
        return "Go to Student Dashboard → Create Resume"

    elif "job" in msg:
        return "Go to Dashboard → View Jobs → Apply"

    elif "login" in msg:
        return "Enter your email and password to login"

    elif "signup" in msg:
        return "Register using email and verify OTP"

    elif "hr" in msg:
        return "HR can post jobs, view resumes, and shortlist candidates"

    elif "contact" in msg or "email" in msg:
        return "Contact us at: Hiresmarttly@gmail.com"
...
def chatbot(request):
    try:
        ...
        # 🔒 Block sensitive
        if is_sensitive(user_msg):
            return JsonResponse({
                "reply": "Sorry, personal data cannot be shared."
            })

        # ⚡ Quick reply
        quick = smart_reply(user_msg)
        if quick:
            return JsonResponse({"reply": quick})
            ...
            messages=[
                {
                    "role": "system",
                    "content": """
You are HireSmart Assistant.
Only answer about HireSmart features.
...
#job and resume bey match thase---------------------------------------
...
def post_job(request):
    ...
    hr = get_object_or_404(HR, email=hr_email)
    ...
    if request.method == "POST":
        ...
        return redirect("/post-job/?posted=1")

    jobs = Job.objects.filter(hr=hr).order_by("-id")
    return render(request, "post_job.html", {"jobs": jobs})
...
def delete_job(request, job_id):
    ...
    hr = get_object_or_404(HR, email=hr_email)
    job = get_object_or_404(Job, id=job_id, hr=hr)

    job.delete()

    return redirect("/post-job/?deleted=1")
...
def update_job(request, job_id):
    ...
    if request.method == "POST":
        ...
        return redirect("/post-job/?updated=1")
...
def approve_with_interview(request):
    ...
    if request.method == "POST":
        ...
        Message.objects.create(
            ...
            interview_date=date,
            interview_time=time_obj   # ✅ USE THIS
        )
...
def view_applicants(request):
    ...
    .filter(job__hr=hr, status="pending")
...
# LANDING PAGE-CIRCLE ANIMAION MATE CHE 
